...
next_hidden, past, attention = m.forward(*m.tokenize(["is", "except"], add_space=True), past, attention)
"""
...
class LanguageModel(nn.Module):
    def __init__(self, pretrained: str = "gpt2"):
        """
        :param pretrained: name of the pretrained model, from ("gpt2", "gpt2-medium", "gpt2-large", "gpt2-xl")
        """
    ...
    def forward(self,
        ...
        :param attention_mask: (batch_size, input_id_len) 1 for tokens that should be attended to, 0 for padding, etc.
        ...
        :return: (last_hidden_state, key_values, new_attention_mask)
                 last_hidden_state: (batch_size, input_id_len, hidden_size) final hidden state associated
                                    with each input_id
    ...
    def tokenize(self, inputs: List[str], add_space: bool = False) -> Tuple[torch.LongTensor, torch.LongTensor]:
        ...
        :param add_space: True if a space should be added to the strings so that the first token is considered
